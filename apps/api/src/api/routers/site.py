...
async def admin_update_settings(
    ...
    before = PublicSiteSettingsOut.model_validate(await site_svc.get_settings(db)).model_dump(
        mode="json"
    )
    ...
    await audit_svc.record(
        ...
        meta={"before": before, "after": data.model_dump(exclude_unset=True, mode="json")},
        summary="更新公開官網設定",
    )
    return settings
...
async def admin_update_link_category(
    ...
    item = await db.get(site_svc.PublicLinkCategory, category_id)
    if not item:
...
async def admin_update_page(
    ...
    page = await db.get(site_svc.PublicSitePage, page_id)
    if not page:
