...
from api.schemas.raffle import RaffleCreate, RaffleDrawRequest
...
async def create_event(db: AsyncSession, body: RaffleCreate, user: User) -> RaffleEvent:
    event = RaffleEvent(
        event_code=body.event_code.strip(),
...
async def join_event(
    ...
    result = await db.execute(
        select(RaffleEvent)
        .options(selectinload(RaffleEvent.prizes))
        .where(RaffleEvent.event_code == event_code.strip())
    )
    event = result.scalar_one_or_none()
    if event is None or not _is_valid_code(access_code, event.access_code_hash):
        raise PermissionError("活動碼或驗證碼不正確")
    if event.status not in {RaffleStatus.OPEN, RaffleStatus.PAUSED}:
        raise ValueError("這場抽獎目前尚未開放")
