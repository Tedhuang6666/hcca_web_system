"""通知偏好正規化測試 — 空 / 舊扁平 / 新巢狀三格式相容（Phase 2）"""
...
def test_normalize_legacy_flat_bool_format() -> None:
    ...
    assert result["document_pending"] == {"inapp": False, "email": False, "line": False}
    # announcement 舊值 True → inapp True；email 因不在預設清單仍 False
    assert result["announcement"] == {"inapp": True, "email": False, "line": False}
...
def test_normalize_new_nested_format_preserved() -> None:
    ...
    assert result["system"] == {"inapp": False, "email": True, "line": False}


def test_normalize_partial_nested_fills_missing_subkeys() -> None:
    result = normalize_preferences({"document_rejected": {"email": True}})
    # 缺 inapp → 補 True
    assert result["document_rejected"] == {"inapp": True, "email": True, "line": False}
