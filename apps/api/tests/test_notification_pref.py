...
def test_normalize_legacy_flat_bool_format() -> None:
    ...
    assert result["document_pending"] == {"inapp": False, "email": False}
    # announcement 舊值 True → inapp True；email 因不在預設清單仍 False
    assert result["announcement"] == {"inapp": True, "email": False}
