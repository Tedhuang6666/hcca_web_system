...
export default function MeetingDetailPageClient({
  params,
  initialMeeting = null,
  initialMinutes = null,
}: {
  params: Promise<{ id: string }>;
  initialMeeting?: MeetingOut | null;
  initialMinutes?: MeetingMinutesOut | null;
}) {
  const [id, setId] = useState("");
  const [meeting, setMeeting] = useState<MeetingOut | null>(initialMeeting);
  const [minutes, setMinutes] = useState<MeetingMinutesOut | null>(initialMinutes);
  ...
  useEffect(() => {
    if (!id) return;
    if (initialMeeting?.id === id) return;
  ...
  useEffect(() => {
    ...
    if (initialMinutes?.meeting.id === id) return;
    meetingsApi.minutes(id).then(setMinutes).catch(() => setMinutes(null));
  }, [id, initialMinutes, meeting?.status]);
