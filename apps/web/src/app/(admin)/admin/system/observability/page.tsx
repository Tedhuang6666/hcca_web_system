...
type PageScore = { url: string; path: string; source?: "psi" | "synthetic" | "configured" | "rum"; status: PageStatus; authenticated_status?: PageStatus; mobile: ModeScore | null; desktop: ModeScore | null; authenticated_mobile: ModeScore | null; authenticated_desktop: ModeScore | null };
...
type RouteTelemetry = { path: string; device_class?: "mobile" | "desktop" | "unknown"; auth_state?: "public" | "authenticated" | "unknown"; release?: string; pageviews: number; api_errors: number; api_timeouts?: number; protective_responses?: number; client_errors?: number; samples: Record<string, number>; web_vitals: Record<string, number | null>; api_latency_p95_ms: number | null; api_latency_percentiles_ms?: Percentiles; api_latency_p95_ms_by_kind?: Record<string, ApiLatencyBudget>; interaction_feedback_p75_ms?: number | null; interaction_completion_p75_ms?: number | null; interaction_samples?: { feedback: number; completion: number }; resource_timing_p75_ms?: number | null; resource_timing_percentiles_ms?: Percentiles; longtask_p75_ms?: number | null; longtask_percentiles_ms?: Percentiles; longtask_samples?: number; navigation_ttfb_p75_ms?: number | null; navigation_ttfb_percentiles_ms?: Percentiles; navigation_total_p75_ms?: number | null; navigation_total_percentiles_ms?: Percentiles; custom_metrics?: Record<string, { p75_ms: number | null; samples: number }>; interactions?: InteractionTelemetry[] };
type RealUsersData = { configured: boolean; data_available: boolean; dau: number | null; sessions: number | null; pageviews: number | null; top_routes: RouteTelemetry[]; web_vitals: Record<string, number | null>; client_errors: number | null; source: string; message: string; window_hours?: number };
...
function statusLabel(status: PageStatus, source?: PageScore["source"]) {
  if (status === "pending" && source === "rum") return "RUM 已發現／待 Lighthouse";
  if (status === "pending" && source === "configured") return "等待 Lighthouse 排程";
...
export default function ObservabilityPage() {
  ...
  const [rumWindowHours, setRumWindowHours] = useState(24);
  ...
  useEffect(() => {
    ...
    const endpoint = tab === "errors"
      ...
      : tab === "real-users"
        ? `/admin/system/observability/real-users?window_hours=${rumWindowHours}`
        : tab === "performance"
    ...
    return () => { active = false; };
  }, [tab, selectedUrl, refreshKey, rumWindowHours]);
...
function RealUsersPanel({ data, windowHours, onWindowHoursChange }: { data: RealUsersData; windowHours: number; onWindowHoursChange: (value: number) => void }) {
  ...
  const routeInteractions = routes.flatMap((route) =>
    (route.interactions ?? []).map((interaction) => ({ route, interaction })),
  );
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 text-xs" style={{ color: "var(--text-muted)" }}>
              時間區間
              <select className="min-h-9 rounded-md border bg-transparent px-2 text-sm" style={{ borderColor: "var(--border-strong)", color: "var(--text-primary)" }} value={windowHours} onChange={(event) => onWindowHoursChange(Number(event.target.value))}>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <SummaryMetric label="Pageviews" value={data.pageviews == null ? "—" : data.pageviews} detail={`第一方 RUM，近 ${data.window_hours ?? windowHours} 小時`} />
          ...
          <div className="overflow-x-auto rounded-md border" style={{ borderColor: "var(--border)" }}>
            <table className="w-full min-w-[1380px] text-left text-sm">
              <thead><tr className="border-b" style={{ borderColor: "var(--border)" }}>
                <th className="px-4 py-3">路由</th><th className="px-4 py-3">裝置／狀態</th><th className="px-4 py-3">版本</th><th className="px-4 py-3">瀏覽</th><th className="px-4 py-3">Web Vitals p50 / p75 / p95 / p99</th>
                <th className="px-4 py-3">互動回饋 / 完成 p75</th><th className="px-4 py-3">API p50 / p75 / p95 / p99</th><th className="px-4 py-3">瀏覽器執行效能</th><th className="px-4 py-3">錯誤</th>
              </tr></thead>
              <tbody>{routes.map((route) => {
                ...
                const webVitals = route.web_vitals ?? {};
                  return <tr key={`${route.path}:${route.device_class}:${route.auth_state}:${route.release}`} className="border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                  ...
                  <td className="px-4 py-3 font-mono text-[11px] leading-5"><div>LCP {formatNumber(webVitals.lcp_p50, " ms")} / {formatNumber(webVitals.lcp_p75, " ms")} / {formatNumber(webVitals.lcp_p95, " ms")} / {formatNumber(webVitals.lcp_p99, " ms")}</div><div>INP {formatNumber(webVitals.inp_p50, " ms")} / {formatNumber(webVitals.inp_p75, " ms")} / {formatNumber(webVitals.inp_p95, " ms")} / {formatNumber(webVitals.inp_p99, " ms")}</div><div>FCP {formatNumber(webVitals.fcp_p50, " ms")} / {formatNumber(webVitals.fcp_p75, " ms")} / {formatNumber(webVitals.fcp_p95, " ms")} / {formatNumber(webVitals.fcp_p99, " ms")}</div><div>CLS {formatNumber(webVitals.cls_p50)} / {formatNumber(webVitals.cls_p75)} / {formatNumber(webVitals.cls_p95)} / {formatNumber(webVitals.cls_p99)}</div><div>TTFB {formatNumber(route.navigation_ttfb_percentiles_ms?.p50_ms, " ms")} / {formatNumber(route.navigation_ttfb_percentiles_ms?.p75_ms, " ms")} / {formatNumber(route.navigation_ttfb_percentiles_ms?.p95_ms, " ms")} / {formatNumber(route.navigation_ttfb_percentiles_ms?.p99_ms, " ms")}</div></td>
                  ...
                  <td className="px-4 py-3 text-xs"><div>GET {formatNumber(api.simple_get?.p50_ms, " ms")} / {formatNumber(api.simple_get?.p75_ms, " ms")}</div><div>GET {formatNumber(api.simple_get?.p95_ms, " ms")} / {formatNumber(api.simple_get?.p99_ms, " ms")}</div><div>CRUD p95 {formatNumber(api.crud?.p95_ms, " ms")} · 重型 {formatNumber(api.heavy?.p95_ms, " ms")}</div></td>
                  ...
                  <td className="px-4 py-3 text-xs" style={{ color: route.api_errors || route.api_timeouts || route.client_errors ? "var(--error)" : "var(--success)" }}><div>HTTP {route.api_errors}</div><div>Timeout {route.api_timeouts ?? 0}</div><div>保護性回應 {route.protective_responses ?? 0}</div><div>瀏覽器 {route.client_errors ?? 0}</div></td>
                </tr>;
              })}</tbody>
            </table>
          </div>
      ...
      {routeInteractions.length > 0 && <section className="space-y-3"><div><h2 className="text-base font-semibold">具名操作</h2><p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>按鈕、表單與導覽操作會量測按下後首次視覺回饋，以及對應 API 完成時間。</p></div><div className="grid gap-3 md:grid-cols-2">{routeInteractions.slice(0, 40).map(({ route, interaction }) => <div key={`${route.path}:${route.device_class}:${route.auth_state}:${route.release}:${interaction.name}`} className="rounded-md border p-4" style={{ borderColor: "var(--border)" }}><div className="flex items-center justify-between gap-3"><span className="truncate text-sm font-medium">{interaction.name}</span><span className="text-xs" style={{ color: "var(--text-muted)" }}>{route.path} · {route.device_class} · {route.auth_state}</span></div><div className="mt-2 grid grid-cols-2 gap-3 text-xs"><div style={{ color: budgetColor(interaction.feedback_status) }}>回饋 {formatNumber(interaction.feedback_p75_ms, " ms")}</div><div style={{ color: budgetColor(interaction.completion_status) }}>完成 {formatNumber(interaction.completion_p75_ms, " ms")}</div></div></div>)}</div></section>}
    </div>
  );
}
