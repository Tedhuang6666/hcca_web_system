  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 py-6">
      ...
      {view === "list" ? (
        ...
        <div className="grid grid-cols-7 overflow-hidden rounded-md border border-[var(--border)]">
                  ...
                  {dayEvents.slice(0, 5).map((event) => (
                    <button
                      ...
                      <span
                        ...
                        aria-hidden="true"
                      />
                      <span className="font-medium">{formatTime(event.starts_at, event.all_day)}</span>{" "}
...
function EventEditor({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div
        className="w-full max-w-lg rounded-md border border-[var(--border)] bg-[var(--background)] p-5 shadow-xl"
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button onClick={onClose} className="rounded-md border border-[var(--border)] px-4 py-2 text-sm">
            取消
          </button>
          <button
            onClick={submit}
...
function EventDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30" onClick={onClose}>
      <aside
        className="ml-auto flex h-full w-full max-w-xl flex-col overflow-y-auto border-l border-[var(--border)] bg-[var(--background)] shadow-xl"
        onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[var(--border)] bg-[var(--background)] px-5 py-4">
          <div className="min-w-0">
            <p className="truncate text-lg font-semibold">{event?.title ?? "行程"}</p>
