...
const DOC_VISIBILITIES: { key: string; label: string }[] = [
  { key: "subject_only", label: "密件" },
  { key: "org_only", label: "機關可見" },
  { key: "public", label: "登入" },
...
export default function DocumentListClient({
  ...
  initialDocs?: DocumentListItem[] | null;
}) {
  ...
  const [filters, dispatchFilter] = useReducer(
    ...
    () => ({
      ...
      visibility:       canViewAll ? searchParams.get("visibility") ?? "" : "",
  ...
  const queryVisibility = canViewAll ? filters.visibility : undefined;
