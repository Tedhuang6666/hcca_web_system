...
import { CheckCircle, ExternalLink, Link2, Link2Off, Loader2, RefreshCw } from "lucide-react";
import { googleTasksApi, type GoogleTasksStatus } from "@/lib/api";
...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    ...
    googleTasksApi.status()
      .then(setStatus)
      .catch(() => toast.error("無法載入整合狀態"))
      .finally(() => setLoading(false));
  }, []);

  async function handleConnect() {
    window.location.href = `${process.env.NEXT_PUBLIC_API_URL ?? ""}/user/google-tasks/authorize`;
  }
