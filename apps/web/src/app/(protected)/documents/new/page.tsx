        ...
        <GuidedFormStep step={3} activeStep={activeStep} className="space-y-4">
          ...
          <FormSection title="可見度">
            <select value={visibilityLevel}
              ...
              style={selectStyle}>
              <option value="subject_only">密件（建立者、上層機關與收件者）</option>
              <option value="org_only">機關可見（全機關）</option>
              <option value="public">登入（登入後可見）</option>
              <option value="publicly_open">公開（未登入可見）</option>
            </select>
          ...
          <div className="card p-4 space-y-2" aria-label="儲存前檢查">
            ...
            <dl className="space-y-1 text-xs" style={{ color: "var(--text-secondary)" }}>
              ...
              <div className="flex items-center justify-between gap-3">
                <dt>可見範圍</dt><dd className="font-medium">{
                  visibilityLevel === "subject_only" ? "密件" :
                  visibilityLevel === "org_only" ? "機關可見" :
                  visibilityLevel === "public" ? "登入" : "公開"
                }</dd>
              </div>
            </dl>
