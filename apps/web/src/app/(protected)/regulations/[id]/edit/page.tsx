...
import { documentsApi, regulationsApi, regulationHref, serialTemplatesApi, apiErrorMessage } from "@/lib/api";
...
type RevisionDatePrecision = "date" | "month" | "year" | "unknown";

type RevisionDraft = {
  id: string;
  changeBrief: string;
  precision: RevisionDatePrecision;
  date: string;
  year: string;
  documentId: string;
};

function revisionDraftFrom(revision: RegulationRevisionOut): RevisionDraft {
  return {
    id: revision.id,
    changeBrief: revision.change_brief,
    precision: revision.amended_at_precision ?? "date",
    date: revision.amended_at_precision === "unknown" ? "" : revision.amended_at.slice(0, 10),
    year: revision.amended_year?.toString() ?? "",
    documentId: revision.published_document_id ?? "",
  };
}
...
export default function EditRegulationPage() {
  ...
  const saveInfo = async () => {
    ...
    try {
      if (reg && !["draft", "rejected"].includes(reg.workflow_status)) {
        const updated = await regulationsApi.updateMetadata(id, {
          effective_date: effectiveDate ? `${effectiveDate}T00:00:00+08:00` : null,
          legislative_history: legislativeHistory || null,
        });
        setReg(updated);
  ...
  const canEditContent = wfStatus === "draft" || wfStatus === "rejected";
  ...
  const saveRevision = async (draft: RevisionDraft) => {
    if (!draft.changeBrief.trim()) {
      toast.error("請填寫沿革摘要");
      return;
    }
    if ((draft.precision === "date" || draft.precision === "month") && !draft.date) {
      toast.error(draft.precision === "date" ? "完整日期精度需要填寫日期" : "年月精度需要填寫日期");
      return;
    }
    if (draft.precision === "year" && !draft.year) {
      toast.error("年份精度需要填寫年份");
      return;
    }
    setSavingRevisionId(draft.id);
    try {
      const updated = await regulationsApi.updateRevision(id, draft.id, {
        change_brief: draft.changeBrief.trim(),
        amended_at: draft.precision === "date" || draft.precision === "month" ? draft.date : null,
        amended_at_precision: draft.precision,
        amended_year: draft.precision === "year" ? Number(draft.year) : null,
        document_id: draft.documentId || null,
      });
      setRevisionDrafts(prev => prev.map(item => item.id === draft.id ? revisionDraftFrom(updated) : item));
      setReg(prev => prev ? {
        ...prev,
        revisions: prev.revisions.map(item => item.id === updated.id ? updated : item),
        published_document_id: prev.published_document_id ?? updated.published_document_id,
      } : prev);
      toast.success(`v${updated.version} 沿革已更新`);
    } catch (e) {
      toast.error(apiErrorMessage(e, "沿革更新失敗"));
    } finally {
          ...
          <div className="min-w-0 lg:col-span-2 space-y-4">
            ...
            <div className="card p-4 space-y-3">
              ...
              <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>
                舊法規可填完整日期、年月、僅年份，或標記日期未知；公布令可連結到對應沿革。
              </p>
              {revisionDrafts.length === 0 ? (
                <p className="text-xs" style={{ color: "var(--text-muted)" }}>尚無可編輯的修訂沿革。</p>
              ) : revisionDrafts.map((draft) => (
                <div key={draft.id} className="rounded-lg p-3 space-y-2" style={{ border: "1px solid var(--border)" }}>
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-semibold" style={{ color: "var(--text-primary)" }}>v{reg.revisions.find(r => r.id === draft.id)?.version}</span>
                    <button
                      onClick={() => saveRevision(draft)}
                      disabled={savingRevisionId === draft.id}
                      className="text-xs px-2.5 py-1 rounded disabled:opacity-50"
                      style={{ color: "var(--primary)", border: "1px solid var(--border-strong)" }}
                    >
                      {savingRevisionId === draft.id ? "儲存中..." : "儲存沿革"}
                    </button>
                  </div>
                  <input
                    value={draft.changeBrief}
                    onChange={e => setRevisionDrafts(prev => prev.map(item => item.id === draft.id ? { ...item, changeBrief: e.target.value } : item))}
                    className="w-full bg-transparent text-xs p-2 rounded outline-none"
                    style={inputStyle}
                    placeholder="沿革摘要"
                  />
                  <div className="grid gap-2 sm:grid-cols-3">
                    <select
                      ...
                      onChange={e => setRevisionDrafts(prev => prev.map(item => item.id === draft.id ? { ...item, precision: e.target.value as RevisionDatePrecision } : item))}
                      className="text-xs rounded px-2 py-2"
                      style={{ background: "var(--bg-elevated)", border: "1px solid var(--border)", color: "var(--text-primary)" }}
                    >
                      <option value="date">完整日期</option>
                      <option value="month">年月</option>
                      <option value="year">僅年份</option>
                    ...
                    {draft.precision === "year" ? (
                      <input
                        ...
                        max="9999"
                        value={draft.year}
                        onChange={e => setRevisionDrafts(prev => prev.map(item => item.id === draft.id ? { ...item, year: e.target.value } : item))}
                        className="w-full bg-transparent text-xs p-2 rounded outline-none"
                        style={inputStyle}
                        placeholder="年份（可填民國年）"
                      />
                    ) : (
                      <input
                        type="date"
                        value={draft.date}
                        disabled={draft.precision === "unknown"}
                        onChange={e => setRevisionDrafts(prev => prev.map(item => item.id === draft.id ? { ...item, date: e.target.value } : item))}
                        className="w-full bg-transparent text-xs p-2 rounded outline-none disabled:opacity-50"
                        style={inputStyle}
                      />
                    )}
                    <select
                      value={draft.documentId}
                      onChange={e => setRevisionDrafts(prev => prev.map(item => item.id === draft.id ? { ...item, documentId: e.target.value } : item))}
                      className="text-xs rounded px-2 py-2"
                      style={{ background: "var(--bg-elevated)", border: "1px solid var(--border)", color: "var(--text-primary)" }}
                    >
            ...
            <div className="card p-4 space-y-3">
              ...
              <div>
                ...
                <SmartTextarea value={preface} onChange={setPreface} rows={2} readOnly={!canEditContent} style={inputStyle} />
              </div>
              <div>
              ...
              <div>
                ...
                <SmartTextarea value={legalBasis} onChange={setLegalBasis} rows={2} readOnly={!canEditContent} style={inputStyle} />
              </div>
              <div>
                <label className="text-xs mb-1 block" style={{ color: "var(--text-muted)" }}>提案/決議資訊</label>
                <SmartTextarea value={proposalMetadata} onChange={setProposalMetadata} rows={2} readOnly={!canEditContent} style={inputStyle} />
              </div>
              <button onClick={saveInfo} disabled={savingInfo}
                ...
                {savingInfo ? "儲存中..." : canEditContent ? "儲存基本資訊" : "儲存沿革與生效日期"}
              </button>
            </div>
