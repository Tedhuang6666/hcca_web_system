...
export default function MerchandiseSubmissionsAdminPage() {
  ...
  const saveSettings = async () => { if (!settings) return; setSavingSettings(true); try { const updated = await merchandiseSubmissionsApi.updateSettings({ is_open: settings.is_open, opens_at: toIso(toLocal(settings.opens_at)), closes_at: toIso(toLocal(settings.closes_at)), max_file_size_mb: settings.max_file_size_mb, announcement: settings.announcement || null }); setSettings(updated); toast.success("全站投稿設定已儲存"); } catch (error) { toast.error(apiErrorMessage(error, "無法儲存全站設定")); } finally { setSavingSettings(false); } };
