...
export const receivablesApi = {
  list: (params?: {
    class_id?: string; user_id?: string; status?: string; limit?: number;
  }) => {
  ...
  summary: (params?: { class_id?: string }) => {
    const q = new URLSearchParams();
    if (params?.class_id) q.set("class_id", params.class_id);
    const qs = q.toString();
  ...
  create: (body: {
    source_type?: ReceivableSource; source_id?: string | null;
  ...
  exportUrl: (params?: { class_id?: string }) => {
    const q = new URLSearchParams();
    if (params?.class_id) q.set("class_id", params.class_id);
    const qs = q.toString();
